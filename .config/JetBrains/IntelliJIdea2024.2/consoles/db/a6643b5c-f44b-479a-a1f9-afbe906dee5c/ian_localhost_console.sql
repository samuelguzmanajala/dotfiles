WITH
    date_range AS (
        SELECT generate_series(
                       ...
                       CAST(:startDate AS date) + CAST(:days AS integer) * interval '1 day' - interval '1 day',
                       '1 day'
               ) AS target_date
    ),
    ...
    forecasts AS (
        ...
        WHERE forecast_date >= CAST(:startDate AS date)
          AND forecast_date <  CAST(:startDate AS date) + CAST(:days AS integer) * interval '1 day'
    ),
...
--                                %s
    ...
    final_calculations AS (
        ...
        FROM final_intermediate fi
            ...
            WHERE need_positive_with_production = 0 AND need_negative_with_production = 0
    )
SELECT
    COUNT(*)                                    AS total_rows,
